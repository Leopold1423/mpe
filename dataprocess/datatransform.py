...
class DataTransform(object):
    def __init__(self, dataset_path, record_path, store_stat=False, seed=2022):
        self.data_path = dataset_path
        self.store_stat = store_stat
        self.seed = seed
        self.stats_path = record_path.joinpath("stats")
        self.feat_map = {}
        self.defaults = {}
        self.save_cnt = {}
    ...
    def generate_and_filter(self, threshold=0, label_index="", white_list=[]):
        ...
        for field in self.field_name:
            ...
            if field not in white_list:
                ...
                default_list = [value for key, value in feat_count.items() if value < threshold]  # cnt
            else:
            ...
            cnt = {i: feat_count[j] for i, j in enumerate(unique_feat)}# cnt
            if default_list:
                cnt[len(unique_feat)] = sum(default_list)
            self.save_cnt.update({field: cnt})
        ...
        with open(self.stats_path.joinpath("save_cnt.pkl"), 'wb') as fi:
            pickle.dump(self.save_cnt, fi)
